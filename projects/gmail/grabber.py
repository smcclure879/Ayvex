...
def getAttachments( msgId, label ):
    ...
    dest: The directory used to store attachments.
    """
    
    #try:

    message = retry(service.users().messages().get(userId=whoami, id=msgId).execute)
    for part in message['payload']['parts']:
        ...
        body=part['body']
        if   'attachmentId' in body   and   'filename' in part   :
            aId=body['attachmentId']
            fileName=part['filename']
            try:
                req2 = retry(service.users().messages().attachments().get(userId=whoami, id=aId, messageId=msgId).execute)
                #{                 #callback(part.filename, part.mimeType, attachment);             })  bugbug
            except Exception as ex:
                print(ex)
                sys.exit();
            if 'data' not in req2:
                print("nope1152x:"+req2)
                sys.exit()
            data=req2['data']
            fileData = base64.urlsafe_b64decode( data )
            msgDir=os.path.join(dest,label+"_"+msgId)
            os.makedirs(msgDir, exist_ok=True)
            fileName=fileName[:-4] + 'p' + myHash(fileData) + fileName[-4:]
            path = os.path.join( msgDir, fileName)
            print( aId, path )
            print( data[1::30] )
            try:
                singleWrite(path,fileData)
            except:
                print(fileData)
                die("check file data bugbug0058d")
                retry(lambda:singleWrite(path,fileData))

            

        elif 'filename' in part['headers'] and part['partId']==0:
            #print(part)
            print("skipping partId0 ",ii)

        elif 'body' in part:
            print("skipping body ",ii)

        else:
            print("skipping part bugbug",part)
            print("bugbug1334")
            sys.exit()
...
startDate = datetime.date(2019,8,26)
endDate = datetime.date(2019,9,30)  #bugbug not futureproof
...
for dd in range(howManyDays.days):
    ...
    ymd="%Y/%m/%d"  #  0420160822AAWR098767096WCVU
    q="0420160822AAWR098767096WCVU   after:%s  before:%s " %   (date1.strftime(ymd),date2.strftime(ymd)),
    ...
    for x in result.get('messages',[]):
        ...
        msgObj=dict()
        res2=retry(service.users().messages().get(userId=whoami,format='full',id=messageId).execute)
        headers=dictify(res2['payload']['headers'])
        if 'Date' not in headers:
            print(res2['payload'])
            print("bugbug700x why no date")
            sys.exit()
        print(headers)
        ...
        hour=date.hour
        mm=date.minute
        label="{}-{:02d}/{:02d}-{:02d}-{:02d}/{:02d}-{:02d}".format(date.year,date.month,
                                                                    date.year,date.month,dom,
                                                                    hour,mm)
        print(messageId,(dom,hour,mm))

        if messageId in ids:
            dupCount+=1
            print("dup",label,messageId)
            print("dupCount",dupCount)
            ids[messageId] += 1
        else:
            ids[messageId] = 1

        retry(lambda: getAttachments(messageId,label))
        print("done, waiting ")
        time.sleep(0.7)
