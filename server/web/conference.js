...
function maybeDoTeleconfInternal(localCopyOfItem,itemFromServer)  {
	...
	alert("missing localCopy err1203");
	return;	
    }else{
	//and continue on
    }
	...
	//"missing or bad otherParty...but MOST items should fail this test
	return;
    }
    ...
    localCopyOfItem.setAttribute('material','color','red');  //bugbug for debugging
	...
	localCopyOfItem.setAttribute('material','color','blue');  //bugbug for debugging
	...
	if (theyAreWhoWeCalled(otherParty))	{
	    localCopyOfItem.setAttribute('material','color','cyan');  //bugbug for debugging

	    finalizeOfferCycle(otherParty);  //sets inCall to 2
	} else {
	    localCopyOfItem.setAttribute('material','color','green');  //bugbug for debugging
	    //nothing
	}
    } else {
...
var initialConferenceTarget=null;
